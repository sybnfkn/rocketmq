...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // JVM只存在一个Manager
                // 维护一个map，key=clientId    value=MQClientInstance
                // instance维护了网络处理api，是消息生产者和消费者和broker，nameserver打交道网络通道
     ...
     * @param isolation true：使用默认时长30s计算broker故障规避时长，false，使用本次消息发送延迟时间计算broker故障规避时长
     */
    ...
    private SendResult sendDefaultImpl(
        ...
        // 查找主题的路由信息
        // 如果生产者缓存了topic路由信息，直接返回
        // 如果没有存活或者没有包含消息队列，泽向nameserver查询topic路由信息
        // 如果最终没找到，抛出异常
    ...
    public void endTransaction(
        ...
        switch (localTransactionState) {
            case COMMIT_MESSAGE: // 提交（执行本地成功）
                requestHeader.setCommitOrRollback(MessageSysFlag.TRANSACTION_COMMIT_TYPE);
                break;
            case ROLLBACK_MESSAGE: // 回滚（1。发送prepare异常   2。执行本地事务异常）
